...
SimpleKCM {
    ...
    readonly property var imageVersions: kcm.imageVersions ?? null
    ...
    ColumnLayout {
        ...
        Kirigami.InlineMessage {
            readonly property bool failed: root.imageVersions?.errorMessage.length > 0

            Layout.fillWidth: true

            type: failed ? Kirigami.MessageType.Error : Kirigami.MessageType.Warning
            text: {
                if (failed) {
                    return root.imageVersions.errorMessage;
                }
                if (root.imageVersions?.incomplete) {
                    return i18nc("@info", "Some system versions are only partly kept, so they can still be deleted. Apply to fix this.");
                }
                return "";
            }
        ...
        SystemVersions {
            ...
            visible: root.imageVersions !== null
            imageVersions: root.imageVersions
        }
    }
}
