...
bool DiscoverNotifier::notifyAboutUpdates() const
{
    if (state() != NormalUpdates && state() != SecurityUpdates) {
        ...
        qDebug(NOTIFIER) << "Not notifying about updates, state is" << state();
        return false;
    }

    if (m_settings->requiredNotificationInterval() < 0) {
        qDebug(NOTIFIER) << "Not notifying about updates, requiredNotificationInterval is" << m_settings->requiredNotificationInterval();
        return false;
    }
    ...
    if (earliestNextNotificationTime.isValid() && earliestNextNotificationTime > QDateTime::currentDateTimeUtc()) {
        qDebug(NOTIFIER) << "Not notifying about updates, earliestNextNotificationTime is" << earliestNextNotificationTime;
        return false;
    }
    ...
    if (QDBusConnection::sessionBus().interface()->isServiceRegistered(QStringLiteral("org.kde.discover"))) {
        qDebug(NOTIFIER) << "Not notifying about updates, discover is running";
        return false;
    }
    return true;
}
...
void DiscoverNotifier::showUpdatesNotification()
{
    ...
    if (!notifyAboutUpdates()) {
        qDebug(NOTIFIER) << "showUpdatesNotification: not notifying about updates";
        return;
    }
    qDebug(NOTIFIER) << "showUpdatesNotification: notifying about updates";
...
void DiscoverNotifier::updateStatusNotifier()
{
    ...
    qDebug(NOTIFIER) << "updateStatusNotifier: hasUpdates" << hasUpdates << "hasSecurityUpdates" << hasSecurityUpdates;
...
void DiscoverNotifier::refreshUnattended()
{
    ...
    if (!notifyAboutUpdates()) {
        qDebug(NOTIFIER) << "refreshUnattended: not notifying about updates";
        return;
    }

    qDebug(NOTIFIER) << "refreshUnattended: useUnattendedUpdates" << m_settings->useUnattendedUpdates() << "isOnline" << isOnline() << "isConnectionAdequate"
    ...
    if (enabled) {
        qDebug(NOTIFIER) << "Enabling unattended updates";
        m_unattended = std::make_unique<UnattendedUpdates>(this);
    } else {
        qDebug(NOTIFIER) << "Disabling unattended updates";
        m_unattended.reset();
    }
}
