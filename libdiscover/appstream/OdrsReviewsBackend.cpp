...
void OdrsReviewsBackend::parseRatings()
{
    ...
    fw->setFuture(QtConcurrent::run([]() -> State {
        ...
        state.sortedAppRatings = {state.ratings.cbegin(), state.ratings.cend()};
        std::ranges::sort(state.sortedAppRatings, std::greater{}, &Rating::ratingCount);

        state.top = calculateTop(state.sortedAppRatings, {}, 25);
        return state;
    }));
}

QList<Rating> OdrsReviewsBackend::calculateTop(const QList<Rating> &ratings, const std::shared_ptr<Category> &category, uint topSize)
{
...
{
    return QtConcurrent::run(calculateTop, m_current.sortedAppRatings, category, pageSize);
}
